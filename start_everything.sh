...
for i in "$@"; do
	xxx=`expr 0 - $i \* 2 - 1490`
	gnome-terminal --tab -- bash -c "sleep 2; ros2 launch mbzirc_ign spawn.launch.py name:=suav_$i world:=coast model:=mbzirc_quadrotor x:=${xxx} y:=0 z:=4.3 R:=0 P:=0 Y:=0 slot0:=mbzirc_hd_camera slot0_rpy:=\"0 20 0\" gripper:=mbzirc_suction_gripper; exec bash"
done
