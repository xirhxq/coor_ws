...
int bUAV_id;
double buav_hold_points[7][3] = {{}, {-1535, -134, 49},
...
public:
    ...
    double hold_time, hold_duration = 1000000.0;
    ...
    bUAV(char *name) : Node("buav_" + std::string(name)) {
        ...
        clock_sub = this->create_subscription<rosgraph_msgs::msg::Clock>(
            "/clock", 10, std::bind(&bUAV::clock_callback, this, _1)
        );
...
private:
    ...
    void StepPrepare(){
        ...
        if (is_near(hold_point, 1)){
            ...
            task_state = HOLD;
            hold_time = get_time_now() - task_begin_time;
        }
    }
    
    void StepHold(){
        printf("Holding: %.2lf!!!\n", task_time - hold_time);
        printf("Hold Start from %.2lf\n", hold_time);
        UAV_Control_to_Point_earth(hold_point);
    ...
    void timer_callback() {
        ...
        printf("Hold @ (%.2lf, %.2lf, %.2lf)\n", buav_hold_points[bUAV_id][0], buav_hold_points[bUAV_id][1], buav_hold_points[bUAV_id][2]);
